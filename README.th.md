...
- **ทำซ้ำได้** ใครก็ได้ที่มี repo และเนื้อหาเดียวกันสามารถสร้างแผนภูมิที่แม่นยำของคุณใหม่ได้ การคำนวณใช้ astronomy-engine สำหรับท้องฟ้า และ iztro สำหรับ Zi Wei Dou Shu — ทั้งคู่กำหนดได้
...
มันรันภายใต้ `set -euo pipefail` และทำหกขั้นตอน:
...
5. **Build pinned iztro bundle** — รัน `scripts/build-iztro-bundle.sh` ซึ่ง install `iztro@2.5.8` ลง temporary build directory และ output `vendor/iztro.cjs`
6. **รัน smoke test** — execute engine หนึ่งครั้งด้วย fixed sample inputs (ทิ้ง stdout) และ print `OK — engine runs.` เมื่อสำเร็จ พร้อม print usage hints สำหรับทั้งสอง modes
...
Python direct dependencies ถูก pin ไว้ใน `requirements.txt`:
...
Zi Wei ใช้ generated Node bundle ที่ pin โดย `scripts/build-iztro-bundle.sh`:
...
ตอนนี้ engine รันบน verified **CPython 3.12** runtime เหตุผลถูกระบุเหมือนกันใน `requirements.txt` และ `setup.sh`:

> CPython 3.12 ยังคง locked สำหรับ change นี้ ข้อจำกัด native dependency ของ Python Zi Wei แบบเก่าหายไปแล้ว จึง revisit ได้หลังตรวจ remaining dependencies และ deployment image

สรุป: 3.12 ยังเป็น runtime ที่ test แล้วสำหรับ release นี้ Zi Wei dependency ไม่ได้ force lock นี้อีกต่อไป ดังนั้นการ bump Python version เป็น follow-up compatibility check ไม่ใช่ส่วนหนึ่งของ refactor นี้
...
| **ตรวจสอบแล้ว** | ความเจิ่มดาว Zi Wei Dou Shu — ปรับตามเอาต์พุต iztro library | ตรวจสอบกับไลบรารี่ |
...
- **ไม่มี Chiron / bodies รอง** adapter แสดงเฉพาะ 10 ดาวเคราะห์คลาสสิกและ lunar nodes
- **Zi Wei Dou Shu ใช้ school ค่าเริ่มต้นเดียว** iztro เรียกด้วยตัวเลือก fixed (`bySolar(..., True, 'zh-TW')`); star-placement school และ 四化 เป็น whatever iztro defaults ไป ถ้าปกติคุณใช้ 飛星 หรือ school อื่น โครงสร้าง principal ไม่เปลี่ยนแปลง แต่รายละเอียดบาง อาจแตกต่าง
...
repo นี้ได้รับอนุญาตภายใต้ **[MIT](./LICENSE)**

engine ใช้ **astronomy-engine (MIT)** สำหรับการคำนวณดาราศาสตร์ และ **iztro (MIT)** สำหรับ Zi Wei Dou Shu สูตรเรือน Placidus และอ้างอิง Meeus อยู่ใน [CREDITS.md](./CREDITS.md)

คุณสามารถใช้ คัดลอก แก้ไข แจกจ่าย ให้สิทธิ์ต่อ และขายสำเนาได้ตามเงื่อนไข MIT โปรดเก็บ copyright และ permission notice ไว้ในส่วนสำคัญของซอฟต์แวร์

ดูการให้เครดิตและ license ของ dependencies ทั้งหมดที่ **[CREDITS.md](./CREDITS.md)**
...
ไม่ใช่ เนื้อหาเป็นวันที่สุริยะแบบ Gregorian (`--date YYYY-MM-DD`) และเวลานาฬิกา (`--time HH:MM`) ถ้าคุณมีเพียงวันที่อักษรจันทรคติ ให้แปลง it first Zi Wei Dou Shu ถูกคำนวณผ่าน iztro solar entry point (`bySolar`)
...
ไม่รวม adapter แสดงเฉพาะ 10 ดาวเคราะห์คลาสสิกและ lunar nodes

**ไหน Zi Wei Dou Shu school ไป?**
Default school เช่น implemented โดย iztro (`bySolar(..., True, 'zh-TW')`). School ไม่ user-selectable
...
**ฉันใช้ในผลิตภัณฑ์ปิดซอร์สได้ไหม?**
ได้ repo นี้ใช้ license MIT โปรดเก็บ copyright และ permission notice ไว้ในส่วนสำคัญของซอฟต์แวร์
...
- **astronomy-engine** — Don Cross, MIT; การคำนวณดาราศาสตร์
- **Meeus, Astronomical Algorithms** — อ้างอิงสูตรเรือน Placidus
- **iztro** — MIT สำหรับ Zi Wei Dou Shu
- การให้เครดิตเต็ม: **[CREDITS.md](./CREDITS.md)**
- **License:** [MIT](./LICENSE)
