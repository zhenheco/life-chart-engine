...
- **पुनरुत्पादक।** रिपो और एक ही इनपुट के साथ कोई भी आपके सटीक चार्ट को पुनः उत्पन्न कर सकता है। गणना आकाश के लिए astronomy-engine और 紫微斗數 के लिए iztro का उपयोग करती है — दोनों नियतात्मक हैं।
...
यह `set -euo pipefail` के अंतर्गत चलता है और छह चरण करता है:
...
5. **Pinned iztro bundle बनाता है** — `scripts/build-iztro-bundle.sh` चलाता है, जो `iztro@2.5.8` को अस्थायी build directory में install करता है और `vendor/iztro.cjs` बनाता है।
6. **Smoke test चलाता है** — fixed sample inputs के साथ engine को एक बार चलाता है (stdout छोड़कर) और सफलता पर `OK — engine runs.` print करता है। यह दोनों modes के usage hints भी print करता है।
...
Python की direct dependencies `requirements.txt` में pinned हैं:
...
Zi Wei `scripts/build-iztro-bundle.sh` द्वारा pinned generated Node bundle का उपयोग करता है:
...
Engine अभी verified **CPython 3.12** runtime पर चलता है। कारण `requirements.txt` और `setup.sh` में समान रूप से लिखा है:

> इस बदलाव के लिए CPython 3.12 locked रहता है। पुरानी Python Zi Wei native dependency constraint हट गई है, इसलिए बाकी dependencies और deployment image जाँचने के बाद इसे फिर देखा जा सकता है।

संक्षेप में: इस release के लिए 3.12 अभी भी tested runtime है। Zi Wei dependency अब यह lock force नहीं करती, इसलिए Python-version bump इस refactor का हिस्सा नहीं बल्कि बाद की compatibility check है।
...
सभी छह जन्म झंडे **आवश्यक** हैं — एक लापता झंडा stderr पर usage के साथ exit `2` करता है और stdout पर कुछ नहीं देता, इसलिए आप कभी उदाहरण व्यक्ति के चार्ट को गलती से अपना चार्ट नहीं समझ सकते। बिल्ट-इन उदाहरण व्यक्ति (`範例`, जन्म `2000-01-01 12:00`, UTC+8, Taipei 101) देखने के लिए स्पष्ट रूप से `--example` पास करें।

> **Breaking change (v1.1.0):** लापता झंडों पर उदाहरण व्यक्ति में पुराना चुप fallback हटा दिया गया है। जो scripts इस पर निर्भर थीं, उन्हें `--example` पास करना चाहिए।

| झंडा | प्रकार | आवश्यक | प्रारूप / नियम |
|------|--------|--------|-------------|
| `--date` | string | **हाँ** | `YYYY-M-D` (शून्य-पैडिंग वैकल्पिक, उदा. `1990-6-15`)। वास्तविक calendar तारीख, वर्ष समर्थित सीमा **1900–2100** के भीतर। |
| `--time` | string | **हाँ** | `H:M`, 24-घंटे स्थानीय घड़ी समय (शून्य-पैडिंग वैकल्पिक, उदा. `8:30`)। |
| `--tz` | float | **हाँ** | DST सहित UTC offset (Taiwan = `8`), `[-12, 14]` के भीतर, finite। Input key `tz_offset` में लिखा गया। |
| `--lat` | float | **हाँ** | दशमलव डिग्री में अक्षांश, `[-90, 90]` के भीतर, finite। |
| `--lon` | float | **हाँ** | दशमलव डिग्री में देशांतर, `[-180, 180]` के भीतर, finite। |
| `--gender` | string | **हाँ** | बिल्कुल `男` या `女` होना चाहिए (紫微 को प्रभावित करता है; कुछ और → exit `2`)। |
| `--example` | झंडा | नहीं | बिल्ट-इन उदाहरण व्यक्ति की गणना करता है। सभी छह जन्म झंडों के साथ mutually exclusive (मिलाने पर → exit `2`); `--name`, `--target`, `--ziwei-day-divide`, `--json` के साथ मिलाया जा सकता है। |
| `--name` | string | नहीं | मुक्त पाठ; आउटपुट में केवल प्रतिध्वनि। डिफ़ॉल्ट `"範例"`। |
| `--target` | string | नहीं | `YYYY-M-D`; 紫微 luck-period संदर्भ तारीख (運限參考日), वही 1900–2100 सीमा। डिफ़ॉल्ट `"2025-01-01"`। |
| `--ziwei-day-divide` | string | नहीं | 晚子時 नियम: `forward` (डिफ़ॉल्ट) 23:00-23:59 को अगले दिन के रूप में गिनता है; `current` इसे वर्तमान दिन के रूप में गिनता है। |
| `--json` | झंडा | नहीं | उपस्थिति → JSON मोड; अनुपस्थिति → Markdown। कोई मान नहीं लेता। |
...
| `ziwei` | `five_elements_class`, `soul`, `body`, `hour_index`, `palaces[]`, `horoscope` (सफलता पर हमेशा `{ decadal, yearly, age }` — horoscope की विफलता पूरे request को loudly fail करती है)। |
| `meta` | `{ engine, version, ephemeris }` — सभी literals (`ephemeris: "astronomy-engine"`). |
...
- **`ziwei.horoscope` all-or-nothing है।** सफलता पर यह हमेशा `{ decadal, yearly, age }` होता है; sidecar/horoscope की विफलता पूरे request को loudly fail करती है (exit `1` / HTTP `500`) — एक आंशिक या `null` horoscope कभी भी `"ok": true` response में नहीं निकलता। `stars` केवल `decadal`/`yearly` के अंतर्गत आता है (कभी `age` नहीं); `yearlyDecStar` केवल `yearly` के अंतर्गत। `mutagen` एक bare star-name array `[str, …4]` है, निश्चित 祿/權/科/忌 क्रम में — **`schema_version` `1.0` से अपरिवर्तित**। `schema_version` `1.1` एक additive, backward-compatible bump है: अपरिवर्तित `mutagen` के साथ यह `decadal`/`yearly`/`age` पर `mutagenTyped` (उसी क्रम में एक typed view `[{ "star", "type" }, …4]`), `decadal.ageRange` `[startAge, endAge]`, और `age` sub-object (小限 / annual minor limit, `null` हो सकता है) जोड़ता है। `mutagenTyped` में positional 祿/權/科/忌 mapping सभी 10 天干 में invariant है। (वे sub-objects प्रलेखित placeholder से परे अतिरिक्त आंतरिक संरचना भी expose करते हैं — `index`, `palaceNames[]`, `heavenlyStem`/`earthlyBranch`, आदि।)
...
| **सत्यापित** | 紫微斗數 तारा brightness — iztro लाइब्रेरी के आउटपुट के साथ संरेखित। | लाइब्रेरी के विरुद्ध सत्यापित। |
...
- **कोई Chiron / मामूली निकाय नहीं।** adapter केवल 10 शास्त्रीय ग्रह और चंद्र nodes output करता है।
- **紫微斗數 एक डिफ़ॉल्ट स्कूल का उपयोग करता है।** iztro को निश्चित विकल्पों (`bySolar(..., True, 'zh-TW')`) के साथ कॉल किया जाता है; तारा-placement स्कूल और 四化 जो भी iztro डिफ़ॉल्ट है। यदि आप सामान्यतः 飛星 या किसी अन्य स्कूल का उपयोग करते हैं, तो मुख्य संरचना अपरिवर्तित है लेकिन कुछ विवरण भिन्न हो सकते हैं।
...
यह रिपॉजिटरी **[MIT](./LICENSE)** के तहत लाइसेंस प्राप्त है।

इंजन खगोलीय गणना के लिए **astronomy-engine (MIT)** और Zi Wei Dou Shu के लिए **iztro (MIT)** का उपयोग करता है। Placidus house formulas और Meeus reference [CREDITS.md](./CREDITS.md) में दिए गए हैं।

MIT शर्तों के तहत आप प्रतियां उपयोग, कॉपी, modify, distribute, sublicense और sell कर सकते हैं। सॉफ्टवेयर के substantial portions में copyright और permission notice बनाए रखें।

पूर्ण attribution और dependency licenses के लिए **[CREDITS.md](./CREDITS.md)** देखें।
...
नहीं। Inputs एक Gregorian solar date (`--date YYYY-MM-DD`) और घड़ी समय (`--time HH:MM`) हैं। यदि आपके पास केवल एक lunar date है, तो पहले इसे convert करें। 紫微斗數 iztro के solar entry point (`bySolar`) के via computed है।
...
शामिल नहीं हैं। adapter केवल 10 शास्त्रीय ग्रह और चंद्र nodes output करता है।

**कौन सा 紫微斗數 स्कूल यह उपयोग करता है?**
iztro द्वारा implemented default स्कूल (`bySolar(..., True, 'zh-TW')`)। स्कूल user-selectable नहीं है।
...
**क्या मैं इसे closed-source product में उपयोग कर सकता हूँ?**
हाँ। यह repository MIT-licensed है। सॉफ्टवेयर के substantial portions में copyright और permission notice बनाए रखें।
...
- **astronomy-engine** — Don Cross, MIT; खगोलीय गणना।
- **Meeus, Astronomical Algorithms** — Placidus house formulas का reference।
- **iztro** — MIT, Zi Wei Dou Shu के लिए।
