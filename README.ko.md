...
이 도구는 **재현 가능하고 검증 가능한** 차트 계산을 원하는 사람들 — 실무자, 자기인식 도구를 구축하는 개발자, 순수 계산 단계가 필요한 AI 에이전트 — 을 위해 만들어졌습니다. 모든 수치는 실제 천문 계산(Swiss Ephemeris)과 실제 자미두수 라이브러리(py-iztro)에서 나오며, 원격 서비스나 캐시된 조회, 네트워크 통신은 전혀 사용하지 않습니다.
...
- **재현 가능.** 리포지토리와 동일한 입력을 가지고 있는 누구나 정확한 차트를 재생성할 수 있습니다. 계산은 하늘을 위해 Swiss Ephemeris(Moshier 모델)를, 자미두수를 위해 py-iztro를 사용 — 둘 다 결정론적입니다.
...
`~/.life-chart-engine`으로 설치됩니다(`LIFE_CHART_DIR`으로 재정의 가능). `sudo` 없음, 시스템 전체 변경 없음 — 리포지토리만 클론하고 격리된 CPython 3.12 venv를 빌드합니다. `git`과 [`uv`](https://docs.astral.sh/uv/)가 필요합니다. 최신 버전으로 업데이트하려면 언제든지 다시 실행하세요.
...
`set -euo pipefail` 아래에서 실행되며 다섯 가지 단계를 수행합니다:
...
5. **스모크 테스트 실행** — 고정된 샘플 입력으로 엔진을 한 번 실행(stdout 폐기)하고 성공 시 `OK — engine runs.`를 출력합니다. 또한 두 모드에 대한 사용 힌트를 출력합니다.
...
유일한 두 개의 직접 의존성은 `requirements.txt`에 고정되어 있습니다:
...
엔진을 **CPython 3.12**에서 실행해야 합니다 — 3.13도, 3.14도 아닙니다. 그 이유는 `requirements.txt`와 `setup.sh`에서 동일하게 명시됩니다:

> py-iztro의 네이티브 의존성(**pythonmonkey / pydantic-core**)은 **3.13+/3.14에 대한 휠이 없고 소스에서 빌드할 수 없습니다**. 3.12로 고정하세요.

요약하면: `py-iztro`는 네이티브 확장(`pythonmonkey`, `pydantic-core`)에 의존하는데, 이들의 사전 빌드 휠은 3.12에서 중단됩니다. 3.13/3.14에는 휠이 없고 소스 빌드가 실패합니다. 이것이 정확히 `setup.sh`가 `uv venv --python 3.12`를 호출하는 이유이며, 시스템 `python3`이 아닌 프로젝트 venv의 Python(`<repo>/.venv/bin/python`)으로 항상 엔진을 호출해야 하는 이유입니다.
...
| **검증됨** | 자미두수 별 밝기 — py-iztro 라이브러리의 출력과 정렬됨. | 라이브러리에 대해 검증됨. |
...
- **자미두수는 하나의 기본 학파를 사용합니다.** py-iztro는 고정된 옵션(`by_solar(..., True, 'zh-TW')`)으로 호출되며, 별 배치 학파와 사화는 py-iztro의 기본값입니다. 보통 비성(飛星) 또는 다른 학파를 사용한다면, 주요 구조는 변경되지 않지만 일부 세부 사항은 다를 수 있습니다.
...
**음력을 입력할 수 있나요?**
아니요. 입력은 그레고리력 태양 날짜(`--date YYYY-MM-DD`)와 시계 시간(`--time HH:MM`)입니다. 음력 날짜만 있는 경우, 먼저 변환하세요. 자미두수는 py-iztro의 태양 진입점(`by_solar`)을 통해 계산됩니다.
...
**어느 자미두수 학파를 사용합니까?**
py-iztro에서 구현한 기본 학파(`by_solar(..., True, 'zh-TW')`). 학파는 사용자 선택 불가.
