...
이 도구는 **재현 가능하고 검증 가능한** 차트 계산을 원하는 사람들 — 실무자, 자기인식 도구를 구축하는 개발자, 순수 계산 단계가 필요한 AI 에이전트 — 을 위해 만들어졌습니다. 모든 수치는 실제 천문 계산과 실제 자미두수 라이브러리(iztro)에서 나오며, 원격 서비스나 캐시된 조회, 네트워크 통신은 전혀 사용하지 않습니다.
...
| **자미두수(Zi Wei Dou Shu)** | 12궁판에 운명을 매핑하는 전통 중국 점성학. 명명된 별들로 채워짐. | 오행국(五行局), 명주(命主, 영혼) / 신주(身主, 신체), 시진(時辰) 시간 인덱스, 그리고 궁별 데이터 — 간지, 명/신 플래그, 10년 나이 범위, 주요/부차/형용사 별들(밝기 및 사화(四化) 포함). 여기에 대한(大限)/유년(流年)/소한(小限) 운세 포함 — 그 사화(四化)는 별 이름만의 배열(`mutagen`)과 추가 제공되는 타입 지정 `{star, type}` 뷰(`mutagenTyped`)로 표현되며, 대한(大限)에는 나이 범위가 포함됨. |
...
- **재현 가능.** 리포지토리와 동일한 입력을 가지고 있는 누구나 정확한 차트를 재생성할 수 있습니다. 계산은 하늘을 위해 astronomy-engine를, 자미두수를 위해 iztro를 사용 — 둘 다 결정론적입니다.
...
`~/.life-chart-engine`에 설치합니다(`LIFE_CHART_DIR`로 재정의 가능). `sudo`도, 시스템 전체 변경도 없습니다 — repo를 clone하고 격리된 CPython 3.12 venv를 만들며 고정된 iztro Node 번들을 생성할 뿐입니다. `git`, [`uv`](https://docs.astral.sh/uv/), 그리고 **Node.js ≥ 18**이 필요합니다(18과 24에서 지원/테스트됨 — 紫微斗數는 Node 사이드카로 실행되며, 사이드카가 없으면 세 번째 시스템을 조용히 누락하는 대신 모든 차트 요청이 명시적으로 실패합니다). 최신 버전으로 업데이트하려면 언제든 다시 실행하세요.
...
여섯 개의 출생 플래그는 모두 **필수**입니다 — 플래그가 하나라도 빠지면 exit `2`로 종료되고 stderr에 usage가 출력되며 stdout에는 아무것도 출력되지 않으므로, 예제 인물의 차트를 자신의 차트로 착각할 일이 없습니다. 기본 제공 예제 인물(`範例`, 출생 `2000-01-01 12:00`, UTC+8, Taipei 101)을 보려면 `--example`을 명시적으로 전달하세요.

> **호환성이 깨지는 변경(v1.1.0):** 플래그 누락 시 예제 인물로 무음 폴백되던 기존 동작이 제거되었습니다. 이에 의존하던 스크립트는 `--example`을 전달해야 합니다.

| 플래그 | 유형 | 필수 | 형식 / 규칙 |
|--------|------|------|-----------|
| `--date` | 문자열 | **예** | `YYYY-M-D`(0 패딩 선택, 예: `1990-6-15`). 실제 달력 날짜여야 하며, 연도는 지원 범위 **1900–2100** 이내. |
| `--time` | 문자열 | **예** | `H:M`, 24시간 로컬 시계 시간(0 패딩 선택, 예: `8:30`). |
| `--tz` | 부동 소수점 | **예** | DST 포함 UTC 오프셋(대만 = `8`), `[-12, 14]` 범위 내 유한 값. 입력 키 `tz_offset`에 기록됨. |
| `--lat` | 부동 소수점 | **예** | 10진 도 단위 위도, `[-90, 90]` 범위 내 유한 값. |
| `--lon` | 부동 소수점 | **예** | 10진 도 단위 경도, `[-180, 180]` 범위 내 유한 값. |
| `--gender` | 문자열 | **예** | 정확히 `男` 또는 `女`여야 함(자미두수에 영향; 다른 값 → exit `2`). |
| `--example` | 플래그 | 아니오 | 기본 제공 예제 인물을 계산합니다. 여섯 개 출생 플래그 전부와 상호 배타적(함께 쓰면 → exit `2`); `--name`, `--target`, `--ziwei-day-divide`, `--json`과는 조합 가능. |
| `--name` | 문자열 | 아니오 | 자유 텍스트; 출력에만 에코됩니다. 기본값 `"範例"`. |
| `--target` | 문자열 | 아니오 | `YYYY-M-D`; 자미두수 운한 참고 날짜(運限參考日), 동일한 1900–2100 범위. 기본값 `"2025-01-01"`. |
| `--ziwei-day-divide` | 문자열 | 아니오 | 晚子時(늦은 자시) 규칙: `forward`(기본값)는 23:00-23:59를 다음 날로 계산; `current`는 당일로 계산. |
| `--json` | 플래그 | 아니오 | 존재 → JSON 모드; 부재 → 마크다운. 값을 취하지 않음. |
...
| `ziwei` | `five_elements_class`, `soul`, `body`, `hour_index`, `palaces[]`, `horoscope` (성공 시 항상 `{ decadal, yearly, age }` — horoscope 실패는 요청 전체를 시끄럽게 실패시킴). |
| `meta` | `{ engine, version, ephemeris }` — 모두 리터럴 (`ephemeris: "astronomy-engine"`). |
...
- **`ziwei.horoscope`는 all-or-nothing입니다.** 성공 시 항상 `{ decadal, yearly, age }`이며, 사이드카/horoscope 실패는 요청 전체를 시끄럽게 실패시킵니다(exit `1` / HTTP `500`) — `"ok": true` 응답에서 부분적이거나 `null`인 horoscope가 나오는 일은 결코 없습니다. `stars`는 `decadal`/`yearly` 아래에만 나타나고(`age`에는 없음), `yearlyDecStar`는 `yearly` 아래에만 나타납니다. `mutagen`은 고정된 祿/權/科/忌 순서의 순수 별 이름 배열 `[str, …4]`이며 — **`schema_version` `1.0` 이후 변경 없음**. `schema_version` `1.1`은 추가만 있는 하위 호환 범프입니다: 변경 없는 `mutagen`과 함께 `decadal`/`yearly`/`age`에 `mutagenTyped`(같은 순서의 타입 뷰 `[{ "star", "type" }, …4]`), `decadal.ageRange` `[startAge, endAge]`, 그리고 `age` 하위 객체(小限 / 연간 소한, `null`일 수 있음)를 추가합니다. `mutagenTyped`의 위치별 祿/權/科/忌 매핑은 10개 천간 전체에서 불변입니다. (이 하위 객체들은 문서화된 자리 표시자를 넘어서는 추가 내부 구조 — `index`, `palaceNames[]`, `heavenlyStem`/`earthlyBranch` 등 — 도 노출합니다.)
...
- **Chiron / 소행성 없음.** 현재 adapter는 10개의 고전 행성과 달의 교점만 출력합니다.
...
사용자는 이 리포지토리의 URL을 복사하고, **자신의** 에이전트 또는 CLI(Claude Code, Hermes, 스크립트 등)가 **사용자의 머신에서 로컬로** 클론하고 실행합니다. 계산은 사용자 측에서 발생합니다. 이 리포지토리는 호스팅된 엔드포인트를 제공하지 않고, 계정도 필요 없으며, **SaaS 통합이 필요하지 않습니다** — 엔진은 상태 비저장, 결정론적, 오프라인 서브프로세스입니다. (브라우저를 선호하는 사용자를 위해 이 엔진 위에 구축된 별도의 [호스팅 제품](https://life.aicycle.cc)이 있습니다 — 이는 별개의 서비스이며 이 리포지토리의 일부가 아닙니다.)

이 리포지토리 자체는 네트워크 서비스로 운영되지 않습니다. MIT 조건에 따라 로컬 사용, 수정, 배포, 호스팅 사용이 허용되며 자세한 내용은 `LICENSE`에 있습니다.
...
이 저장소는 **[MIT](./LICENSE)** 라이선스로 배포됩니다.

엔진은 천문 계산에 **astronomy-engine (MIT)**, 자미두수 계산에 **iztro (MIT)** 를 사용합니다. Placidus 하우스 공식과 Meeus 참고 문헌은 [CREDITS.md](./CREDITS.md)에 정리되어 있습니다.

MIT 조건에 따라 사용, 복사, 수정, 배포, 재라이선스, 판매가 가능합니다. 소프트웨어의 실질적인 부분에는 copyright 및 permission notice를 유지하세요.

전체 저작자 표시와 의존성 라이선스는 **[CREDITS.md](./CREDITS.md)** 를 참고하세요.
...
**Chiron / 소행성 / 소행성은 어디에?**
포함되지 않습니다. 현재 adapter는 10개의 고전 행성과 달의 교점만 출력합니다.
...
**클로즈드 소스 제품 안에서 사용할 수 있나요?**
예. 이 저장소는 MIT 라이선스입니다. 소프트웨어의 실질적인 부분에는 copyright 및 permission notice를 유지하세요.
...
- **astronomy-engine** — Don Cross, MIT; 천문 계산.
- **Meeus, Astronomical Algorithms** — Placidus 하우스 공식 참고.
- **iztro** — MIT, 자미두수용.
- 전체 저작자 표시: **[CREDITS.md](./CREDITS.md)**.
- **라이선스:** [MIT](./LICENSE).
- **에이전트 / JSON 계약:** [AGENTS.md](./AGENTS.md).
