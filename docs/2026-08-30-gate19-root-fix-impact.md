...
19-49 接上的「情緒」中心本來就已被閘門 49 那側定義、本來就是 motor，因此該連通分量的
motor 集合與對「喉」的可達性都沒有改變；而 Authority 階層中情緒最高，該分量本來就含情緒。
新增的「根」是 motor 但不改變上述兩項判定，所以 Type 與 Authority 不動。
