...
| **紫微斗數** | 運命を12宮盤にマッピングする伝統的な中国占星術システム、名前付きの星で満たされている。 | 五行局、命主（ソウル）/ 身主（ボディ）、時辰インデックス、および宮ごとのデータ — 干支、命/身フラグ、10年年齢範囲、主星/副星/修正星（明度と四化付き）。さらに大限/流年/小限ホロスコープも含みます — その四化は星名のみの配列（`mutagen`）に加えて、追加の型付き `{star, type}` ビュー（`mutagenTyped`）としても出力され、大限には年齢範囲が付きます。 |
...
6つの出生フラグはすべて**必須**です — フラグが欠けていると終了コード `2` で終了し、usage を stderr に出力し、stdout には何も出力しません。したがって、サンプル人物のチャートを自分のものと取り違える心配はありません。組み込みサンプル人物（`範例`、生年月日 `2000-01-01 12:00`、UTC+8、台北101）を見るには、`--example` を明示的に指定してください。

> **破壊的変更（v1.1.0）：** フラグ欠落時にサンプル人物へサイレントにフォールバックする旧動作は削除されました。これに依存していたスクリプトは `--example` を渡してください。

| フラグ | 型 | 必須 | 形式 / ルール |
|-------|----|------|------------|
| `--date` | 文字列 | **はい** | `YYYY-M-D`（ゼロパディングは任意、例：`1990-6-15`）。実在する暦日で、年はサポート範囲 **1900–2100** 内。 |
| `--time` | 文字列 | **はい** | `H:M`、24時間ローカルクロック時刻（ゼロパディングは任意、例：`8:30`）。 |
| `--tz` | float | **はい** | 夏時間を含む UTC オフセット（台湾 = `8`）、`[-12, 14]` の範囲内、有限値。入力キー `tz_offset` に書き込まれます。 |
| `--lat` | float | **はい** | 十進法の緯度、`[-90, 90]` の範囲内、有限値。 |
| `--lon` | float | **はい** | 十進法の経度、`[-180, 180]` の範囲内、有限値。 |
| `--gender` | 文字列 | **はい** | 正確に `男` または `女` である必要があります（紫微に影響；その他 → 終了 `2`）。 |
| `--example` | フラグ | いいえ | 組み込みサンプル人物を計算します。6つの出生フラグとは相互排他（併用 → 終了 `2`）；`--name`、`--target`、`--ziwei-day-divide`、`--json` とは併用可能。 |
| `--name` | 文字列 | いいえ | 自由テキスト。出力にのみエコーされます。デフォルト `"範例"`。 |
| `--target` | 文字列 | いいえ | `YYYY-M-D`；紫微運限参考日、同じ 1900–2100 の範囲。デフォルト `"2025-01-01"`。 |
| `--ziwei-day-divide` | 文字列 | いいえ | 晚子時ルール：`forward`（デフォルト）は 23:00-23:59 を翌日として数え、`current` は当日として数えます。 |
| `--json` | フラグ | いいえ | 存在 → JSON モード；非存在 → Markdown。値を取りません。 |
...
| `ziwei` | `five_elements_class`、`soul`、`body`、`hour_index`、`palaces[]`、`horoscope`（成功時は常に `{ decadal, yearly, age }` — horoscope の失敗はリクエスト全体を明示的に失敗させます）。 |
...
- **`ziwei.horoscope` はオール・オア・ナッシングです。** 成功時は常に `{ decadal, yearly, age }` です；sidecar / horoscope の失敗はリクエスト全体を明示的に失敗させます（終了 `1` / HTTP `500`）— `"ok": true` のレスポンスで部分的な horoscope や `null` の horoscope が出力されることはありません。`stars` は `decadal`/`yearly` の下にのみ現れます（`age` には決して現れません）；`yearlyDecStar` は `yearly` の下にのみ現れます。`mutagen` は素のスター名配列 `[str, …4]` で、祿/權/科/忌 の固定順 — **`schema_version` `1.0` から変更なし**。`schema_version` `1.1` は追加的で後方互換のバンプです：変更のない `mutagen` に加えて、`decadal`/`yearly`/`age` に `mutagenTyped`（同じ順序の型付きビュー `[{ "star", "type" }, …4]`）、`decadal.ageRange` `[startAge, endAge]`、および `age` サブオブジェクト（小限／年次マイナーリミット、`null` の可能性あり）が追加されます。`mutagenTyped` における位置ベースの 祿/權/科/忌 マッピングは、全10天干にわたって不変です。（これらのサブオブジェクトはドキュメント化されたプレースホルダーを超えた追加の内部構造 — `index`、`palaceNames[]`、`heavenlyStem`/`earthlyBranch` など — も公開します。）
...
ユーザーがこのリポジトリの URL をコピーし、**彼ら自身の**エージェント、CLI（Claude Code、Hermes、スクリプト等）が**ユーザーのマシンでローカルに**それをクローンして実行します。計算はユーザー側で発生します。**本リポジトリ**はホストされたエンドポイントを公開せず、アカウントも **SaaS 統合も不要です** — エンジンはステートレス、決定論的、オフラインのサブプロセスです。（ブラウザでの利用を好む方向けに、このエンジンをベースにした独立の[ホスト版プロダクト](https://life.aicycle.cc)が存在します — これは別個のサービスであり、本リポジトリの一部ではありません。）

**本リポジトリ自体**はネットワークサービスとして運用されていません。MIT の下では、ローカル利用、変更、配布、ホスト利用が `LICENSE` の条件で許可されます。
