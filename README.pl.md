...
- **Powtarzalne.** Każdy, kto ma repozytorium i te same dane wejściowe, może regenerować twój dokładny wykres. Obliczenia używają astronomy-engine dla nieba i iztro dla 紫微斗數 — oba deterministyczne.
...
- **Brak Chirona / drobnych ciał.** Adapter zwraca tylko 10 klasycznych planet i węzły Księżyca.
...
Użytkownik kopiuje URL tego repozytorium i **jego własny** agent lub CLI (Claude Code, Hermes, skrypt itp.) klonuje go i uruchamia **lokalnie na maszynie użytkownika**. Obliczenia odbywają się po stronie użytkownika. To repozytorium nie udostępnia hostowanego punktu końcowego, nie wymaga konta i **nie wymaga integracji SaaS** — silnik jest stateless, deterministycznym, offline'owym podprocesem. (Dla osób preferujących przeglądarkę istnieje osobny [hostowany produkt](https://life.aicycle.cc) zbudowany na tym silniku — to odrębna usługa, nie część tego repozytorium.)

Samo to repozytorium nie jest prowadzone jako usługa sieciowa. Na MIT lokalne użycie, modyfikacje, dystrybucja i użycie hostowane są dozwolone zgodnie z `LICENSE`.
...
To repozytorium jest licencjonowane na **[MIT](./LICENSE)**.

Silnik używa **astronomy-engine (MIT)** do obliczeń astronomicznych oraz **iztro (MIT)** do Zi Wei Dou Shu. Formuły domów Placidus i odniesienie do Meeusa są opisane w [CREDITS.md](./CREDITS.md).

Możesz używać, kopiować, modyfikować, dystrybuować, sublicencjonować i sprzedawać kopie na warunkach MIT. Zachowaj copyright i permission notice w istotnych częściach oprogramowania.

Pełna atrybucja i licencje zależności: **[CREDITS.md](./CREDITS.md)**.
...
Nie uwzględnione. Adapter zwraca tylko 10 klasycznych planet i węzły Księżyca.
...
**Czy mogę używać go w produkcie zamkniętoźródłowym?**
Tak. Repozytorium ma licencję MIT. Zachowaj copyright i permission notice w istotnych częściach oprogramowania.
...
- **astronomy-engine** — Don Cross, MIT; obliczenia astronomiczne.
- **Meeus, Astronomical Algorithms** — odniesienie dla formuł domów Placidus.
- **iztro** — MIT, dla Zi Wei Dou Shu.
- Pełna atrybucja: **[CREDITS.md](./CREDITS.md)**.
