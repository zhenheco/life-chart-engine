...
- **Powtarzalne.** Każdy, kto ma repozytorium i te same dane wejściowe, może regenerować twój dokładny wykres. Obliczenia używają Swiss Ephemeris (model Moshiera) dla nieba i py-iztro dla 紫微斗數 — oba deterministyczne.
...
Instaluje do `~/.life-chart-engine` (przesłoń zmienną `LIFE_CHART_DIR`). Bez `sudo`, bez zmian całego systemu — tylko klonuje repozytorium i buduje izolowane venv CPython 3.12. Wymaga `git` i [`uv`](https://docs.astral.sh/uv/). Uruchom ponownie w dowolnym momencie, aby zaktualizować do najnowszej wersji.
...
Działa pod `set -euo pipefail` i wykonuje pięć kroków:
...
5. **Uruchamia test dymu** — wykonuje silnik raz ze stałymi przykładowymi danymi wejściowymi (odrzucając stdout) i drukuje `OK — engine runs.` po powodzeniu. Drukuje również wskazówki użycia dla obu trybów.
...
Jedynymi dwoma bezpośrednimi zależnościami przypisuanymi w `requirements.txt` są:
...
Musisz uruchomić silnik na **CPython 3.12** — nie 3.13, nie 3.14. Powód jest stwierdzony identycznie w `requirements.txt` i `setup.sh`:

> Natywne zależności py-iztro (**pythonmonkey / pydantic-core**) **nie mają kół dla 3.13+/3.14 i nie budują się ze źródła**. Przypnij do 3.12.

Krótko: `py-iztro` zależy od natywnych rozszerzeń (`pythonmonkey`, `pydantic-core`), których prekompilowane koła zatrzymują się na 3.12. Na 3.13/3.14 nie ma kół i budowanie ze źródła nie powiedzie się. To dokładnie dlatego `setup.sh` wymaga `uv venv --python 3.12` i dlatego powinieneś zawsze wywoływać silnik z Pythonem venv projektu (`<repo>/.venv/bin/python`), nigdy nie z systemowym `python3`.
...
| **Zweryfikowana** | Jasność gwiazd 紫微斗數 — dostosowana do wyników biblioteki py-iztro. | Zweryfikowana względem biblioteki. |
...
- **紫微斗數 używa jednej domyślnej szkoły.** py-iztro jest wywoływane z ustalonymi opcjami (`by_solar(..., True, 'zh-TW')`); szkoła rozmieszczenia gwiazd i 四化 są takie, jakie domyślnie ustawia py-iztro. Jeśli zwykle używasz 飛星 lub innej szkoły, główna struktura jest niezmieniona, ale niektóre szczegóły mogą się różnić.
...
1. Zainstaluj (wymaga `git` i `uv`; jeśli uv brakuje: curl -LsSf https://astral.sh/uv/install.sh | sh):
   curl -fsSL https://raw.githubusercontent.com/zhenheco/life-chart-engine/main/install.sh | bash
...
Nie. Dane wejściowe to data gregoriańska słoneczna (`--date YYYY-MM-DD`) i czas zegara (`--time HH:MM`). Jeśli masz tylko datę księżycową, najpierw ją skonwertuj. 紫微斗數 jest obliczany poprzez punkt wejścia słoneczny py-iztro (`by_solar`).
...
Domyślną szkołę zaimplementowaną przez py-iztro (`by_solar(..., True, 'zh-TW')`). Szkoła nie jest wybierana przez użytkownika.
...
- **py-iztro** (i upstream `iztro`) — MIT, dla 紫微斗數.
