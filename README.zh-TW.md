...
它是為想要**可重現、可驗證**的圖表計算的人設計的，而不是黑盒網頁應用：從業者、開發自我認知工具的開發者，以及需要純計算步驟的 AI 代理。每個數字都來自真實的天文計算 (Swiss Ephemeris) 和真實的紫微斗數程式庫 (py-iztro) — 不來自遠端服務、不來自快取查詢，也從不透過網路。
...
- **可重現。** 任何人拿著這個倉庫和相同的輸入都可以重新生成你的確切圖表。計算使用 Swiss Ephemeris (Moshier 模型) 作為天空資料，用 py-iztro 作為紫微斗數 — 兩個都是確定性的。
...
安裝到 `~/.life-chart-engine` (用 `LIFE_CHART_DIR` 覆蓋)。無需 `sudo`、無系統級變更 — 它只是複製倉庫並建立一個隔離的 CPython 3.12 虛擬環境。需要 `git` 和 [`uv`](https://docs.astral.sh/uv/)。任何時候重新執行以更新到最新版本。
...
它在 `set -euo pipefail` 下執行，執行五個步驟：
...
5. **執行冒煙測試** — 使用固定的範例輸入執行引擎一次 (丟棄 stdout) 並在成功時列印 `OK — engine runs.`。它也會列印兩種模式的使用提示。
...
唯一的兩個直接依賴項固定在 `requirements.txt` 中：
...
你必須在 **CPython 3.12** 上執行引擎 — 不是 3.13、不是 3.14。原因在 `requirements.txt` 和 `setup.sh` 中的敘述相同：

> py-iztro 的原生依賴項 (**pythonmonkey / pydantic-core**) **沒有 3.13+/3.14 的 wheel，並且無法從原始碼建立**。固定在 3.12。

簡言之：`py-iztro` 依賴原生擴充 (`pythonmonkey`、`pydantic-core`)，其預建 wheel 停止在 3.12。在 3.13/3.14 上沒有 wheel，原始碼建立失敗。這正是 `setup.sh` 呼叫 `uv venv --python 3.12` 的原因，也是為什麼你應該總是用專案虛擬環境的 Python (`<repo>/.venv/bin/python`) 呼叫引擎，永遠不用系統 `python3`。
...
| **已驗證** | 紫微斗數星曜廟陷 — 與 py-iztro 程式庫輸出對齐。 | 根據程式庫驗證。 |
...
- **紫微斗數使用一個預設學派。** py-iztro 用固定選項呼叫 (`by_solar(..., True, 'zh-TW')`)；星曜位置學派和四化是 py-iztro 預設的。如果你通常使用飛星或另一個學派，主要結構不變但某些細節可能不同。
...
1. 安裝 (需要 `git` 和 `uv`；如果 uv 遺失：curl -LsSf https://astral.sh/uv/install.sh | sh):
   curl -fsSL https://raw.githubusercontent.com/zhenheco/life-chart-engine/main/install.sh | bash
...
**這個倉庫為什麼是 AGPL。** 引擎透過 `pyswisseph` 鏈結 **Swiss Ephemeris**，用於行星位置和宮位尖度。Astrodienst **雙重授權** Swiss Ephemeris 為 **AGPL-3.0 或商業授權**，其代碼不能重新授權為任何更寬容的東西。因為 AGPL 是 copyleft 且這個專案鏈結它，整個組合工作必須是 AGPL。(`py-iztro` 是 MIT 且施加無 copyleft；Swiss Ephemeris 是唯一強制 AGPL 的元件。)
...
3. **交換星曆** — 用寬容授權的天文學來源取代 `pyswisseph` (例如 **Skyfield (MIT)** 加上公有領域 **JPL DE440** 星曆 — 說明替代方案，不是唯一選項)。Swiss Ephemeris 消失，剩餘堆疊 (py-iztro MIT，加上 MPL-2.0/MIT/Apache 依賴項) 不再強制 AGPL，整個倉庫可以是 MIT。這是真實的工程工作：你必須重新實作目前來自 Swiss Ephemeris 的一切 — 行星經度、逆行旗標、上升/中天、Placidus 宮位尖度，以及人類圖 88° 設計解算器的輸入。
...
**我可以輸入陰曆日期嗎？**
否。輸入是格里高曆太陽日期 (`--date YYYY-MM-DD`) 和時鐘時間 (`--time HH:MM`)。如果你只有陰曆日期，先轉換它。紫微斗數透過 py-iztro 的太陽進入點 (`by_solar`) 計算。
...
**它使用哪個紫微斗數學派？**
py-iztro 實作的預設學派 (`by_solar(..., True, 'zh-TW')`)。學派不是使用者可選的。
