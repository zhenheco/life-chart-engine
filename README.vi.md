...
- **Có thể lặp lại.** Bất kỳ ai có repo và cùng đầu vào có thể tạo lại biểu đồ chính xác của bạn. Các phép tính sử dụng astronomy-engine cho bầu trời và iztro cho 紫微斗數 — cả hai đều xác định.
...
Nó chạy dưới `set -euo pipefail` và thực hiện sáu bước:
...
5. **Xây dựng bundle iztro đã ghim** — chạy `scripts/build-iztro-bundle.sh`, cài `iztro@2.5.8` vào thư mục build tạm thời và xuất `vendor/iztro.cjs`.
6. **Chạy smoke test** — chạy engine một lần với input mẫu cố định (bỏ stdout) và in `OK — engine runs.` khi thành công. Nó cũng in gợi ý sử dụng cho cả hai chế độ.
...
# 3. Xây dựng bundle iztro đã ghim
...
Các phụ thuộc trực tiếp của Python được ghim trong `requirements.txt`:
...
Zi Wei dùng một bundle Node được sinh và ghim bởi `scripts/build-iztro-bundle.sh`:
...
Engine hiện chạy trên runtime **CPython 3.12** đã được xác minh. Lý do được nêu giống nhau trong `requirements.txt` và `setup.sh`:

> CPython 3.12 vẫn được khóa cho thay đổi này. Ràng buộc phụ thuộc native Python Zi Wei cũ đã biến mất, nên có thể xem xét lại sau khi kiểm tra các phụ thuộc còn lại và image triển khai.

Tóm lại: 3.12 vẫn là runtime đã kiểm thử cho bản phát hành này. Phụ thuộc Zi Wei không còn buộc khóa đó, nên nâng phiên bản Python là một kiểm tra tương thích sau này, không phải một phần của refactor này.
...
| **Được xác minh** | Độ sáng sao 紫微斗數 — được căn chỉnh với đầu ra của thư viện iztro. | Được xác minh so với thư viện. |
...
- **Không có Chiron / các cơ thể nhỏ.** Adapter chỉ xuất 10 hành tinh cổ điển và các nút Mặt Trăng.
- **紫微斗數 sử dụng một trường phái mặc định.** iztro được gọi với các tùy chọn cố định (`bySolar(..., True, 'zh-TW')`); trường phái đặt sao và 四化 là bất cứ gì iztro mặc định. Nếu bạn thường sử dụng 飛星 hoặc một trường phái khác, cấu trúc chính không thay đổi nhưng một số chi tiết có thể khác.
...
Nhà phát hành không vận hành nó như một dịch vụ mạng. Theo MIT, sử dụng cục bộ, sửa đổi, phân phối và sử dụng hosted đều được phép theo `LICENSE`.
...
Repository này được cấp phép theo **[MIT](./LICENSE)**.

Engine dùng **astronomy-engine (MIT)** cho tính toán thiên văn và **iztro (MIT)** cho Zi Wei Dou Shu. Công thức nhà Placidus và tham chiếu Meeus được ghi trong [CREDITS.md](./CREDITS.md).

Bạn có thể sử dụng, sao chép, sửa đổi, phân phối, cấp phép lại và bán bản sao theo điều khoản MIT. Giữ copyright và permission notice trong các phần đáng kể của phần mềm.

Xem **[CREDITS.md](./CREDITS.md)** để biết attribution đầy đủ và giấy phép phụ thuộc.
...
Không. Đầu vào là một ngày mặt trời Gregorian (`--date YYYY-MM-DD`) và thời gian đồng hồ (`--time HH:MM`). Nếu bạn chỉ có một ngày âm lịch, hãy chuyển đổi nó trước. 紫微斗數 được tính toán qua điểm nhập cảnh mặt trời của iztro (`bySolar`).
...
Không bao gồm. Adapter chỉ xuất 10 hành tinh cổ điển và các nút Mặt Trăng.

**Trường phái 紫微斗數 nào mà nó sử dụng?**
Trường phái mặc định như được triển khai bởi iztro (`bySolar(..., True, 'zh-TW')`). Trường phái không thể chọn bởi người dùng.
...
**Tôi có thể dùng nó trong sản phẩm mã nguồn đóng không?**
Có. Repository này dùng giấy phép MIT. Giữ copyright và permission notice trong các phần đáng kể của phần mềm.
...
- **astronomy-engine** — Don Cross, MIT; tính toán thiên văn.
- **Meeus, Astronomical Algorithms** — tham chiếu cho công thức nhà Placidus.
- **iztro** — MIT, cho Zi Wei Dou Shu.
- Attribution đầy đủ: **[CREDITS.md](./CREDITS.md)**.
- **Giấy phép:** [MIT](./LICENSE).
- **Hợp đồng agent / JSON:** [AGENTS.md](./AGENTS.md).
